(function () {
  ...
  const FEEL_SCORE = { Rough: 1, Tired: 2, Okay: 3, Good: 4, Strong: 5 };
  ...
  function renderProfileSummary(containerEl) {
    const rows = [
      ...
      ["Cancer type", load("profile-cause", "")],
      ["Diagnosis date", load("profileDiagnosisDate", "")],
      ["Surgery / procedure date", load("profileSurgeryDate", "")],
      ["Treatment start date", load("profileTracheostomyDate", "")],
      ["Treatment phase", load("profile-communication", "")],
      ["Oncologist", load("profileEnt", "")],
      ["Primary care physician", load("profileSlp", "")],
    ].filter(([, v]) => v);
  ...
  function renderTracking() {
    ...
    renderStatTiles(document.getElementById("tracking-stats"), [
      ...
      { label: "Symptom entries logged this month", value: String(thisMonthEntries.length) },
      { label: "Treatment care days this week", value: `${daysCleaned} / 7` },
    ]);

    // Daily symptom/energy bar chart
    ...
    if (scored.length === 0) {
      renderEmpty(voiceWrap, "No symptom entries logged yet — save a Daily Tracker entry to see this chart.");
    } else {
