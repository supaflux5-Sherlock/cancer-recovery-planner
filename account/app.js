(function () {
  ...
  if (window.CRPAuth) window.CRPAuth.trackEvent("app_opened");
  ...
  function setupDatedForm(config) {
    ...
    document.querySelector(`[data-save="${config.saveKey}"]`).addEventListener("click", () => {
      ...
      renderHistory(entries, config.historyId, config.renderEntry);
      config.reset();
      if (window.CRPAuth) window.CRPAuth.trackEvent(`saved_${config.saveKey}_entry`);
    });
  }
  ...
  setupDatedForm({
    ...
    renderEntry(e) {
      ...
      return `<div class="entry-date">${e.date}</div>
        <div>${e.feel ? "Feeling: <strong>" + e.feel + "</strong>" : ""}</div>
  ...
  function setupTable(tableId, storageKey, columns, addKey, onChange) {
    ...
    document.querySelector(`[data-add="${addKey}"]`).addEventListener("click", () => {
      ...
      persist();
      render();
      if (window.CRPAuth) window.CRPAuth.trackEvent(`added_${addKey}_row`);
    });
  }
  ...
  setupTable(
    ...
    [
      { key: "date", type: "date" },
      { key: "provider", placeholder: "Provider / facility" },
      { key: "item", placeholder: "What it was for" },
      { key: "amount", placeholder: "Amount billed / owed" },
      { key: "notes", placeholder: "Notes (claim #, follow-up needed, etc.)" },
    ],
    "insurance"
  );
  ...
  const MILESTONES = [
    ...
    "First clean scan or good lab result",
    "Halfway through the treatment plan",
    "Last chemo or radiation session",
    "Ringing the bell / end of active treatment",
    "First full day without treatment-related side effects",
    "Return to a favorite activity",
    "First 3-month follow-up scan clear",
  ...
  document.getElementById("export-btn").addEventListener("click", () => {
    ...
    a.href = url;
    a.download = "cancer-recovery-planner-data.json";
    a.click();
